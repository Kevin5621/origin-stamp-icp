...
export class UserService {
  ...
  static async getUserCollections(_username: string): Promise<Collection[]> {
    await new Promise(resolve => setTimeout(resolve, 300));
    return mockCollections.filter(collection => collection.creator.username === _username);
  }
  ...
  static async getUserDashboardStats(_username: string) {
    ...
    const userNFTs = mockNFTs.filter(nft => nft.creator.username === _username);
    const userCollections = mockCollections.filter(collection => collection.creator.username === _username);
    ...
    return {
      ...
      activeListings: userNFTs.filter(nft => nft.status === 'for_sale').length,
      totalViews: userNFTs.reduce((sum, nft) => sum + nft.views, 0),
      totalLikes: userNFTs.reduce((sum, nft) => sum + nft.likes, 0),
      averagePrice: userNFTs.length > 0 ? (userNFTs.reduce((sum, nft) => sum + parseFloat(nft.price.amount), 0) / userNFTs.length).toFixed(2) : '0'
    };
  }
   ...
   * Update user profile
   */
  static async updateUserProfile(_username: string, profileData: Partial<User>): Promise<User> {
   ...
   */
  static async isFollowing(_follower: string, _following: string): Promise<boolean> {

import type { NFT, User, Collection, FilterOptions, SearchResult, CreateNFTData } from '../types/marketplace';
import { mockNFTs, mockUsers, mockCollections, mockCategories, mockMarketplaceStats } from '../utils/mockData';
...
export class MarketplaceService {
  ...
  static async getNFTs(filters: Partial<FilterOptions> = {}): Promise<NFT[]> {
    ...
    switch (filters.sortBy) {
      case 'newest':
        ...
        break;
      case 'oldest':
        filteredNFTs.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
        break;
      case 'price_low':
        filteredNFTs.sort((a, b) => parseFloat(a.price.amount) - parseFloat(b.price.amount));
        break;
      case 'price_high':
        filteredNFTs.sort((a, b) => parseFloat(b.price.amount) - parseFloat(a.price.amount));
        break;
      case 'popular':
  ...
  static async searchNFTs(query: string): Promise<SearchResult> {
    ...
    const nfts = mockNFTs.filter(nft => 
    ...
    const collections = mockCollections.filter(collection =>
   ...
   * Buy NFT
   */
  static async buyNFT(nftId: string, _buyerId: string): Promise<{ success: boolean; transactionId?: string; error?: string }> {
  ...
  static validateFiles(files: FileList): { valid: File[]; invalid: { file: File; reason: string }[] } {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (!allowedExtensions.includes(fileExtension)) {
        invalid.push({ file, reason: 'File type not supported. Use JPG, PNG, GIF, or WebP' });
        continue;
      }

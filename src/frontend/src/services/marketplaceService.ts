...
interface BackendTokenMetadataAttribute { trait_type: string; value: string }
...
interface BackendTokenOwner { owner: { owner: unknown } }
...
export class MarketplaceService {
  ...
  console.debug("[MarketplaceService] Fetched token IDs", tokenIds.map(t=>t.toString()));
      ...
      const nftPromises = tokenIds.map(async (tokenId) => {
        try {
          ...
          console.debug("[MarketplaceService] Token detail loaded", tokenId.toString());
  ...
  const sessionId: string = (token.session_id && token.session_id.length > 0 ? token.session_id[0] : "") || "";

    return {
      id: token.id.toString(),
      title: token.metadata.name || `NFT #${token.id.toString()}`,
      description,
      imageUrl: imageUrl || `https://via.placeholder.com/600x600?text=NFT+${token.id.toString()}`,
      creator: {
        username: String((token.owner as any)?.owner?.toString?.() || "unknown").slice(0, 10) + "...",
        avatar: "",
        verified: true,
      },
  ...
  static async getCollections(): Promise<Collection[]> { return []; }
  static async searchNFTs(_q: string): Promise<SearchResult> { return { nfts: [], collections: [], users: [], total: 0, hasMore: false }; }
}

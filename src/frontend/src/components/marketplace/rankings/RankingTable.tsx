            ...
            <div key={collection.id} className="ranking-card">
              ...
              <div className="ranking-card__main">
                <div className="ranking-card__collection">
                  <img src={collection.image} alt={collection.name} className="ranking-card__image" />
                  <div className="ranking-card__info">
                    <div className="ranking-card__name">{collection.name}</div>
                    <div className="ranking-card__creator">{collection.creator}</div>
                  </div>
                </div>
                <div className="ranking-card__stats">
                  <div className="ranking-card__stat">
                    <span className="ranking-card__label">{t("rankings.table.volume", "Volume")}</span>
                    <span className="ranking-card__value">{collection.volume.toFixed(2)} ETH</span>
                  </div>
                  <div className="ranking-card__stat">
                    <span className="ranking-card__label">{t("rankings.table.floor_price", "Floor")}</span>
                    <span className="ranking-card__value">{collection.floorPrice.toFixed(2)} ETH</span>
                  </div>
                  <div className="ranking-card__stat">
                    <span className="ranking-card__label">{t("rankings.table.percent_change", "Change")}</span>
                    <span className={`ranking-card__change ${
                      collection.percentChange >= 0 ? "ranking-card__change--positive" : "ranking-card__change--negative"
                    }`}>
                      {collection.percentChange >= 0 ? "+" : ""}{collection.percentChange.toFixed(2)}%
                    </span>
                  </div>
                </div>
              </div>

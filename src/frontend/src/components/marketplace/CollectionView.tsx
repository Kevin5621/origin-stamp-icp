...
export const CollectionView: React.FC<CollectionViewProps> = ({
  ...
  const [filters, setFilters] = useState<FilterOptions>({
    categories: ['art', 'photography', 'music', 'video', 'collectible', 'virtual_world'],
  ...
  const loadCollectionData = async () => {
    try {
      ...
      const collectionData: CollectionInfo = {
        ...
        description: "A collection of futuristic digital art pieces exploring themes of technology and human connection. Each piece represents a unique vision of the future and our relationship with the digital realm.",
        bannerImage: "https://images.unsplash.com/photo-1605721911519-3dfeb3be25e7?q=80&w=1200",
        avatarImage: "https://images.unsplash.com/photo-1620510625142-b45cbb784397?q=80&w=300",
        creator: {
          name: "DigitalDreamer",
          address: "0x3b96...a28d",
      ...
      const nftItemsData: NFTItem[] = [
        ...
        },
        {
          ...
          owner: {
            name: "NFTEnthusiast",
            avatar: "https://images.unsplash.com/photo-1603415526960-f7e0328c63b1?q=80&w=100",
          },
          isNew: true,
        },
        {
  ...
  const applyFiltersAndSort = () => {
    ...
    filtered = filtered.filter(
      (item) =>
        ...
        (filters.priceRange.max === null || item.price <= filters.priceRange.max)
    );
    
    // Apply status filters
    if (filters.status.buyNow || filters.status.onAuction || filters.status.new || filters.status.hasOffers) {
  ...
  const handleFollowCollection = (collection: CollectionInfo, isFollowing: boolean) => {
    console.log(`${isFollowing ? 'Following' : 'Unfollowed'} collection:`, collection.title);
              ...
              <div className="collection-view__no-results">
                ...
                <button 
                  ...
                  onClick={() => handleFilterChange({
                    ...
                    status: { buyNow: false, onAuction: false, new: false, hasOffers: false },
                  })}
                >
                  {t("reset_filters")}
                </button>

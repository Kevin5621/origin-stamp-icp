...
interface CheckoutFormProps {
  ...
  onCheckout: (paymentMethod: 'icp' | 'card', paymentDetails: any) => Promise<void>;
  loading?: boolean;
}
  ...
  const updateCardDetails = (field: string, value: string) => {
    ...
    if (errors[`card${field.charAt(0).toUpperCase() + field.slice(1)}`]) {
      setErrors(prev => {
        ...
        delete newErrors[`card${field.charAt(0).toUpperCase() + field.slice(1)}`];
        return newErrors;
      });
    }
  };
    ...
    <div className="checkout-form">
      <form onSubmit={handleSubmit} className="checkout-form__form">
            ...
            {items.map((item) => (
              <div key={item.nft.id} className="checkout-form__order-item">
                ...
                <div className="checkout-form__item-details">
                  <h4 className="checkout-form__item-title">{item.nft.title}</h4>
          ...
          <div className="checkout-form__payment-methods">
            <label className="checkout-form__payment-option">
              ...
              <div className="checkout-form__payment-content">
                <Wallet size={24} />
                <div>
                  <span className="checkout-form__payment-title">ICP Wallet</span>
            ...
            <label className="checkout-form__payment-option">
              ...
              <div className="checkout-form__payment-content">
                <CreditCard size={24} />
                <div>
                  <span className="checkout-form__payment-title">{t('credit_card')}</span>
                  <span className="checkout-form__payment-subtitle">
        ...
        {checkoutData.paymentMethod === 'card' && (
          <div className="checkout-form__section">
            <h4 className="checkout-form__subsection-title">{t('card_details')}</h4>
            
            <div className="checkout-form__card-fields">
              <div className="checkout-form__field">
                <label className="checkout-form__label">{t('card_number')}</label>
                <input
                  type="text"
              ...
              <div className="checkout-form__field-row">
                <div className="checkout-form__field">
                  <label className="checkout-form__label">{t('expiry_date')}</label>
                  <input
                    type="text"
                  ...
                  />
                  {errors.cardExpiry && (
                    <span className="checkout-form__error">{errors.cardExpiry}</span>
                  )}
                </div>
                ...
                <div className="checkout-form__field">
                  ...
                  />
                  {errors.cardCvv && (
                    <span className="checkout-form__error">{errors.cardCvv}</span>
                  )}
                </div>
              </div>
              
              <div className="checkout-form__field">
                <label className="checkout-form__label">{t('cardholder_name')}</label>
                <input
                  type="text"
              ...
              {checkoutData.walletAddress ? (
                <div className="checkout-form__wallet-connected">
                  ...
                  <code className="checkout-form__wallet-address">
                    {checkoutData.walletAddress.slice(0, 8)}...{checkoutData.walletAddress.slice(-6)}
                  </code>
                </div>
              ) : (
        ...
        >
          {loading ? (
            ...
            <>
              <Calculator size={20} />
              <span>{t('complete_purchase')} • {total.toFixed(3)} ICP</span>
            </>
          )}
        </button>
      </form>
